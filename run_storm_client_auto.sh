...
RESOLVER_WATCH_FILE="${STORM_RESOLVER_WATCH_FILE:-${ACTIVE_RESOLVERS_FILE}}"
RESOLVER_WATCH_INTERVAL="${STORM_RESOLVER_WATCH_INTERVAL:-2.0}"
...
exec "${PYTHON_BIN}" "${BASE_DIR}/storm_client.py" \
  ...
  --resolvers-watch-file "${RESOLVER_WATCH_FILE}" \
  --resolvers-watch-interval "${RESOLVER_WATCH_INTERVAL}" \
  --resolvers-watch-min "${RESOLVER_MIN_SELECTED}" \
  --resolvers-watch-max "${RESOLVER_MAX_SELECTED}" \
  --resolvers ${SELECTED}
